...
class Spacy:
    ...
    def ufo(self, canvas, center, radius, number=None):
        ...
        # num = os.listdir('/home/shabaz/Documents/misc/ufo_data/images_temp/')
        # with open('/home/shabaz/Documents/misc/ufo_data/annotations_temp.csv', "a+") as file:
                # writer = csv.writer(file)
                # writer.writerow([len(num), center[0], center[1]-radius//3, radius//2, radius//2, number])
        return canvas
    ...
    def draw(self):
        while True:
            ...
            # num = os.listdir('/home/shabaz/Documents/misc/ufo_data/images_temp/')
            # cv2.imwrite('/home/shabaz/Documents/misc/ufo_data/images_temp/{}.jpg'.format(len(num)) , canvas)
            # f = open("/home/shabaz/Documents/misc/ufo_data/annotations.txt", "w+")
            # f.write( "{} \n".format(str(x).encode(), str(y-radius//3).encode(), str(radius//2).encode(), str(radius//2).encode()))
            # f.close()
            # with open('/home/shabaz/Documents/misc/ufo_data/annotations.csv', "a+") as file:
                # writer = csv.writer(file)
                # writer.writerow([len(num), x, y-radius//3, radius//2, radius//2])
